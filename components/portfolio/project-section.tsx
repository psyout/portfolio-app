      ...
      <div className={`mx-auto max-w-240 border-b border-portfolio-line ${tone === 0 ? "border-t" : ""}`}>
        ...
        >
          <div className="col-start-1 row-start-1 min-w-0">
            <span className="mb-1.5 block text-[10px] font-bold uppercase tracking-[.14em] text-portfolio-title">{project.number} · {project.type}</span>
            <span className="title-font block text-[clamp(21px,2.2vw,28px)] font-semibold leading-tight tracking-[-.035em]">{project.title}</span>
          </div>

          <ul className="col-span-2 col-start-1 row-start-2 m-0 flex list-none flex-wrap gap-2 p-0 min-[760px]:col-span-1 min-[760px]:col-start-2 min-[760px]:row-start-1" aria-label={`${project.title} technology stack`}>
            {project.details.map((technology) => (
              <li className="rounded-full border border-[#b7c7cc] bg-[#b7c7cc]/30 px-3 py-1.5 text-[11px] font-semibold text-portfolio-text/75 transition-colors duration-300 even:border-[#7ab8cc]/70 even:bg-[#7ab8cc]/18 [&:nth-child(3n)]:border-[#859195]/55 [&:nth-child(3n)]:bg-[#859195]/15 group-hover:border-[#4c7380]/55" key={technology}>{technology}</li>
            ))}
          </ul>

          <span className="col-start-2 row-start-1 justify-self-end text-portfolio-title min-[760px]:col-start-3" aria-hidden="true">
        ...
        <div className="border-t border-portfolio-line pb-12 pt-8">
          <div className="grid gap-5 px-1 pb-8 min-[760px]:grid-cols-[.45fr_1.55fr] min-[760px]:items-start min-[760px]:gap-12">
            <p className="m-0 border-l-4 border-[#7ab8cc] py-1 pl-4 text-[11px] font-bold uppercase tracking-[.14em] text-portfolio-title">Case study · {project.number}</p>
            <p className="m-0 max-w-[620px] text-[clamp(17px,2vw,21px)] font-medium leading-[1.55] tracking-[-.015em] text-portfolio-text/75">{project.intro}</p>
          </div>
          ...
          <div className="grid grid-cols-[minmax(220px,.62fr)_minmax(0,1.38fr)] gap-[clamp(44px,7vw,84px)] px-1 pt-10 max-[760px]:grid-cols-1 max-[760px]:gap-8">
            <div>
              <p className="m-0 text-[11px] font-bold uppercase tracking-[.14em] text-portfolio-title">Role &amp; stack</p>
              <p className="mb-0 mt-3 text-[14px] leading-[1.7] text-portfolio-text/60">{project.type}<br />Development &amp; interface design</p>
              <div className="mt-6 border-t border-portfolio-line pt-5">
                <TechnologyStack items={project.details} project={project.title} />
              </div>
            </div>
            <div>
              <p className="m-0 text-[11px] font-bold uppercase tracking-[.14em] text-portfolio-title">Overview</p>
              <p className="mb-0 mt-4 text-[16px] leading-[1.8] text-portfolio-text/70">{project.story}</p>
              <div className="mt-7 flex flex-wrap gap-3">
                {project.liveUrl && (
                  <a className={darkButton} href={project.liveUrl} target="_blank" rel="noreferrer">View live site <ArrowUpRight size={15} aria-hidden="true" /></a>
                )}
                {project.githubUrl && (
                  <a className={darkButton} href={project.githubUrl} target="_blank" rel="noreferrer">Source code <ArrowUpRight size={15} aria-hidden="true" /></a>
                )}
              </div>
            </div>
          </div>
