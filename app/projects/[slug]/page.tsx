					...
					<div className='mx-auto max-w-240'>
						...
						<header className='grid gap-6 pb-10 pt-10 min-[760px]:grid-cols-[.65fr_1.35fr] min-[760px]:items-end'>
							<div>
								...
								<h1 className='title-font mb-0 mt-3 text-[clamp(38px,8vw,72px)] font-semibold leading-[.95] tracking-[-.05em]'>{project.title}</h1>
							</div>
						...
						<div className='grid grid-cols-[minmax(0,7fr)_minmax(200px,3fr)] gap-x-[clamp(44px,7vw,84px)] pt-12 max-[760px]:grid-cols-1 max-[760px]:gap-y-8'>
							<div>
								...
								<p className='mb-0 mt-4 text-pretty text-[17px] leading-[1.7] text-portfolio-text/70'>{project.story}</p>
							</div>
							<div>
						...
						<div className='mt-9 flex flex-wrap gap-3'>
							{project.liveUrl && (
								<a className={darkButton} href={project.liveUrl} target='_blank' rel='noreferrer'>
									View live site <ArrowUpRight size={15} aria-hidden='true' />
								</a>
							)}
							{project.githubUrl && (
								<a className={darkButton} href={project.githubUrl} target='_blank' rel='noreferrer'>
